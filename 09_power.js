...
나머지를 구하는 이유는 계산 결과가 컴퓨터로 나타낼 수 있는 수의 범위를 넘을 수 있기 때문입니다. 하지만 모든 연산이 끝난 뒤에 그 결과를 94,906,249로 나누려고 해서는 안 됩니다. 
...
function power(base, exponent) {
    ...
    // 반으로 쪼갠 거듭제곱의 값을 저장 (트리의 leaf까지 재귀)
    const temp = power(base, half);
    // 거듭제곱끼리 곱한다
    const result = (temp * temp) % 94906249;
    // 지수가 홀수면 base에 거듭제곱끼리 곱한 값을 곱하기
