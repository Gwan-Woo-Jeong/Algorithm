...
function solution(n, words) {
    ...
    for (let i = 1; i < words.length; i++) {
        ...
        let current = words[i];
        // 이전 단어와 현재 단어가 이어지고 사용한 단어가 아니면 계속 진행
        if (before[before.length - 1] === current[0] && used.indexOf(current) < 0) {
            used.push(current);
        } else {
            // 아니면, [번호, 차례]를 리턴
...
let output1 = solution(3, ["tank", "kick", "know", "wheel", "land", "dream", "mother", "robot", "tank"]); // ->  [3,3]
console.log(output1);

let output2 = solution(5, ["hello", "observe", "effect",
    "take", "either", "recognize", "encourage", "ensure", "establish", "hang",
    "gather", "refer", "reference", "estimate", "executive"]); // -> [0,0]
console.log(output2);

let output3 = solution(2, ["hello", "one", "even", "never", "now", "world", "draw"]); // -> [1,3]
console.log(output3);
