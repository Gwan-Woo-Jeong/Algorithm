...
function solution(m, n, board) {
  ...
  while (true) {
    for (let y = 0; y < n; y++) {
      for (let x = 0; x < m; x++) {
        if (rotated[y][x] !== "-" && check(y, x)) mark(y, x);
      }
    }

    if (erase() === 0) return answer;
  }
  ...
  function erase() {
    ...
    rotated.forEach((_, i) => {
      ...
      const erased = m - filtered.length;
      cnt += erased;
...
let output1 = solution(4, 5, ["CCBDE", "AAADE", "AAABF", "CCBBF"]); // -> 14
console.log(output1);

let output2 = solution(6, 6, [
  "TTTANT",
  "RRFACC",
  "RRRFCC",
  "TRRRAA",
