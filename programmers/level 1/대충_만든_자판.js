...
휴대폰의 자판은 하나의 키에 여러 개의 문자가 할당될 수 있다. 
...
이 휴대폰 자판은 키의 개수가 1개부터 최대 100개까지 있을 수 있으며, 특정 키를 눌렀을 때 입력되는 문자들도 무작위로 배열되어 있다. 
...
각 문자열을 작성하기 위해 키를 최소 몇 번씩 눌러야 하는지 순서대로 배열에 담아 return 하는 solution 함수를 완성하라.
...
2번 키 한 번 → B
2번 키 한 번 → B
따라서 총합인 4를 두 번째 인덱스에 저장.
결과적으로 [9,4]를 return.

입출력 예 #2
"B"의 경우, 'B'가 어디에도 존재하지 않기 때문에 -1을 첫 번째 인덱스에 저장.
결과적으로 [-1]을 return.
...
2번 키 두 번 → S
1번 키 한 번 → A
따라서 총합인 4를 첫 번째 인덱스에 저장.

"BGZ"의 경우,
2번 키 한 번 → B
1번 키 두 번 → G
1번 키 세 번 → Z
따라서 총합인 6을 두 번째 인덱스에 저장.
결과적으로 [4, 6]을 return.
 */
...
function solution(keymap, targets) {
  ...
  keymap.forEach((key) => {
    key.split("").forEach((k, index) => {
      const found = kMap.get(k);
      kMap.set(found <= index + 1 ? found : k, index + 1);
    });
  });

  return targets.map(
    (target) =>
      target.split("").reduce((acc, cur) => acc + kMap.get(cur), 0) || -1
  );
}

let output1 = solution(["ABACD", "BCEFD"], ["ABCD", "AABB"]); // -> [9, 4]
console.log(output1);

let output2 = solution(["AA"], ["B"]); // -> [-1]
console.log(output2);

let output3 = solution(["AGZ", "BSSS"], ["ASA", "BGZ"]); // -> [4, 6]
console.log(output3);
